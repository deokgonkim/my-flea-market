...
export const handler = wrapHandler(async (event: APIGatewayProxyEvent) => {
  ...
  console.log("Liking item with slug:", slug);
  ...
  const newItem = await itemService.likeItem(item.id);
  const adminUsers = await telegramUserService.getAdminUsers();
  for (const user of adminUsers) {
    await telegramService.sendMessage(user.telegramUserId, `Item liked: ${item.name} (Slug: ${item.slug})`);
  }
